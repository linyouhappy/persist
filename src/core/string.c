...
//  字符串转大写
void strupp(u_char *dst, u_char *src, size_t n) {
    while (n) {
        *dst = strcupp(*src);
        dst++;
        src++;
...
static uint base64_decode_internal(string_p dst, string_p src, const u_char *basis) {
    size_t          len;
    u_char         *d, *s;
...
uint base64_decode(string_p dst, string_p src) {
    ...
    return base64_decode_internal(dst, src, basis64);
}
